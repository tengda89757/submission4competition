...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[
    ...
    \node[box,fill=ReportPaleBlue] (n) {Navigation state \(N\)\\all live objects + robot};
    ...
    \node[box,fill=ReportPurple!12,right=6mm of g] (select) {Selective commit\\merge by ownership};
    \draw[flow] (n) -- (init);
    \draw[flow] (init) -- (g);
    \draw[flow] (g) -- (select);

    \node[box,fill=ReportPaleOrange,below=8mm of init,text width=42mm] (fromg)
      {from \(G\): target object + robot state};
    \node[box,fill=ReportPaleBlue,below=8mm of g,text width=42mm] (fromn)
      {from \(N\): every non-target material state};
    \draw[commit] (select.south) |- (fromg.east);
    \draw[commit] (select.south) |- (fromn.east);
    ...
    \node[
      ...
      inner ysep=2.4mm,
      font=\sffamily\bfseries\scriptsize
    ] (committed) {Committed live state preserves continuity across repeated L5 cycles};
    \draw[commit] (fromg.south) |- (committed.west);
    \draw[commit] (fromn.south) |- (committed.east);
  \end{tikzpicture}%
  }
