...
if ($Canonical) {
    ...
    if ($Level -eq "L5") {
        # Clear the tightly packed row front-to-back. This prevents a carried
        # center/back tote from sweeping through a still-unmoved neighbour on the
        # southeast departure arc; scoring requires all three, not a fixed order.
        $safeOrder = @(
            "white_tote_b01_left_front",
            "white_tote_b01_left_center",
            "white_tote_b01_left_back"
        )
        $objs = @($safeOrder | Where-Object { $_ -in $configuredObjects })
        $objs += @($configuredObjects | Where-Object { $_ -notin $objs })
