...
$env:ROBOT_AGENT_SPEED_PROFILE = "safe"
...
foreach ($lvl in $Levels) {
    ...
    $rlArgs = @("-ExecutionPolicy","Bypass","-NoProfile","-File",(Join-Path $PSScriptRoot "run_level.ps1"),
                "-Level",$lvl,"-Canonical","-ScriptedOnly","-Score")
    ...
    & powershell.exe -ExecutionPolicy Bypass -NoProfile -File (Join-Path $PSScriptRoot "package_submission.ps1") `
        -Level $lvl -Team $Team `
        -MethodSummary "Geometry-calibrated scripted dual-arm grasp; persistent physics scene; 0.70 m/s bounded navigation; continuous grasp transport; physics-generated trajectory; zero post-editing" `
