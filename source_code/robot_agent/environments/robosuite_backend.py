...
# Navigation ignores only the ground by default. Scene fixtures and unheld
# materials are real obstacles; broad name-based exclusions previously hid
# contacts with neighbouring totes and containers.
...
def _load_robot_params() -> dict:
    ...
    _defaults: dict = {
        ...
        "navigation": {
            ...
            "drive_mode": "direct",
            "control_freq": 20,
            "transport_retreat_distance": 1.0,
            "transport_lane_distance": 2.2,
            "transport_turn_min_object_clearance": 0.65,
        },
        "grasp_policy": {
        ...
        "place": {
            ...
            "output_turn_staging_distance": 1.0,
        },
    }
    ...
    _ranges: dict[tuple[str, str], tuple[float, float]] = {
        ...
        ("navigation", "transport_retreat_distance"): (0.50, 2.00),
        ("navigation", "transport_lane_distance"): (1.00, 4.00),
        ("navigation", "transport_turn_min_object_clearance"): (0.40, 1.50),
        ...
        ("place", "output_turn_staging_distance"): (0.50, 2.00),
    }
...
class RobosuiteBackend:
    ...
    def pick_object(self, target: str) -> bool:
        """Reject the legacy kinematic pick API.

        Competition executions must use :meth:`grasp_object_physics`, which
        requires bilateral finger contact and a successful physical lift.
        """
        logger.error(
            "pick_object is disabled for physical runs; use grasp_object_physics(%r)",
            target,
        )
        return False
    ...
    def _update_held_crate_position(self) -> None:
        """Synchronize the continuous gripper-relative transport constraint."""
        try:
            from robosuite.environments.factory_sorting.transport_attachment import (
                sync_transport_attachment,
            )
            sync_transport_attachment(self.env)
        except Exception as exc:
            logger.warning("_update_held_crate_position failed: %s", exc)

    def place_object(self, target: str) -> bool:
        """Reject the legacy kinematic place API."""
        logger.error(
            "place_object is disabled for physical runs; use place_object_physics(%r)",
            target,
        )
        return False
    ...
    def place_object_physics(self, target: str) -> bool:
        ...
        # Record pre-place frame before any output approach maneuver.
        self._record_trajectory_frame()

        # Read turn and place params from robot_params.json.
        _tp = self._rp["turn"]
        _pp = self._rp["place"]
        output_key = station_name or target

        # L5 places three large totes on one table. Turning at the final base
        # pose makes the carried tote sweep a wide arc through earlier drops.
        # Retreat along the final approach ray, turn in open space, then drive
        # straight in with the tote continuously constrained between the
        # grippers. No object pose is reset during this maneuver.
        final_base_xy: np.ndarray | None = None
        if output_key == "aux_output_1":
            current_xy, _ = self.get_base_pose()
            approach_vec = np.asarray(target_xy, dtype=float) - current_xy
            approach_norm = float(np.linalg.norm(approach_vec))
            if approach_norm < 0.25:
                logger.error(
                    "place_object_physics: unsafe/degenerate output approach, distance=%.3f",
                    approach_norm,
                )
                return False
            approach_dir = approach_vec / approach_norm
            final_base_xy = current_xy.copy()
            staging_xy = final_base_xy - (
                float(_pp["output_turn_staging_distance"]) * approach_dir
            )
            self._mark_trajectory_event(
                "place_staging_start",
                object_name=self._held_crate_name,
                target=output_key,
                staging_xy=[float(staging_xy[0]), float(staging_xy[1])],
            )
            if not self.follow_path([staging_xy]):
                logger.error("place_object_physics: failed to reach output staging point")
                return False
        ...
        # Turn once, at the safe staging pose for L5 or in place otherwise.
        try:
            ...
            logger.error("turn_to_face error: %s", exc)
            return False
        if not result.get("success"):
            logger.error(
                "turn_to_face failed: final_error=%.4f",
                result.get("final_error", -1),
            )
            return False

        if final_base_xy is not None:
            if not self.follow_path([final_base_xy]):
                logger.error("place_object_physics: failed final straight output approach")
                return False
            self._mark_trajectory_event(
                "place_staging_end",
                object_name=self._held_crate_name,
        ...
        # Record post-turn / post-approach frame.
        self._record_trajectory_frame()
        ...
        try:
            ...
            if use_attachment:
                # Lower the closed grippers and object together. The former
                # implementation changed only attachment.world_z while restoring
                # the arms to their original height, visibly separating the object
                # from the fingers before release.
                ...
                eef_goals = {
                    arm: start + np.array([0.0, 0.0, target_z - float(start_qpos[2])])
                    for arm, start in eef_starts.items()
                }
                for step in range(lower_steps):
                    alpha = float(step + 1) / float(lower_steps)
                    targets = {
                        arm: eef_starts[arm] + alpha * (eef_goals[arm] - eef_starts[arm])
                        for arm in col.ARMS
                    }
                    ...
                    raw.step(action)
                    attachment["world_z"] = float(
                        start_qpos[2] + (target_z - start_qpos[2]) * alpha
                    )
                    sync_transport_attachment(raw)
                    self._record_trajectory_frame()
                    if not self._headless:
                        raw.render()
                # Hold the physically reached lower posture while opening.
                turn_posture = _capture_upper_body_posture(raw, robot)
            else:
                for step in range(lower_steps):
                    alpha = float(step + 1) / float(lower_steps)
    ...
    def _animate_arm_reach(self, steps: int = 8) -> None:
        """Deprecated non-physical animation entry point; always disabled."""
        raise RuntimeError("visual-only joint interpolation is disabled")
    ...
    def turn_base_to_yaw_physics(self, target_yaw: float) -> bool:
        """Continuously rotate the base and an actively grasped payload."""
        from robosuite.environments.factory_sorting.turn_to_station import (
            turn_to_face_xy,
        )

        raw = self.env
        base_xy, _ = self.get_base_pose()
        target_xy = base_xy + np.array(
            [math.cos(float(target_yaw)), math.sin(float(target_yaw))], dtype=float
        )
        ...
        try:
            result = turn_to_face_xy(
                ...
                render_sleep=0.0,
                sync_attachment=True,
                post_step_callback=_record,
            )
        except Exception:
            logger.exception("turn_base_to_yaw_physics failed")
            return False
        self._record_trajectory_frame()
        if not result.get("success", False):
            logger.error(
                "turn_base_to_yaw_physics did not converge: error=%.4f",
                result.get("final_error", -1.0),
            )
            return False
        if getattr(raw, "has_judge_collision", False):
            logger.error("turn_base_to_yaw_physics triggered a scene collision")
            return False
        return True
...
def _navigation_collisions(
    ...
    try:
        ...
        if attachment and attachment.get("active", False):
            held_object = attachment.get("object_name")
    except Exception:
        pass

    material_names = sorted(
        list(getattr(env, "material_metadata", {}).keys()),
        key=len,
        reverse=True,
    )

    def _material_owner(geom_name: str | None) -> str | None:
        text = geom_name or ""
        for material_name in material_names:
            if text == material_name or text.startswith(f"{material_name}_"):
    ...
    for contact in env.sim.data.contact[: env.sim.data.ncon]:
        ...
        owner1 = _material_owner(g1)
        owner2 = _material_owner(g2)
        if held_object and (
            (owner1 == held_object and owner2 not in (None, held_object))
            or (owner2 == held_object and owner1 not in (None, held_object))
        ):
        ...
        if held_object and (other == held_object or (other or "").startswith(f"{held_object}_")):
            continue
    ...
    *,
    ...
    for step in range(max_steps):
        ...
        if _should_stop_for_collision(
            ...
            logger.error("follow_path stopped on collision at step %d", step)
            return False
