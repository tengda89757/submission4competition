...
def _default_args(col):
    ...
    fast = os.environ.get("ROBOT_AGENT_SPEED_PROFILE", "safe").strip().lower() == "fast"
    # The collector defaults are deliberately conservative.  In the verified
    # fast profile, keep the same Cartesian waypoints and closed-loop settle
    # checks while reducing interpolation / gripper-hold steps.  Any residual
    # pose error is still handled by the collector's feedback settle loops.
    scale = 0.50 if fast else 1.0
    return argparse.Namespace(
        up_steps=max(20, round(col.DEFAULT_UP_STEPS * scale)),
        xy_steps=max(40, round(col.DEFAULT_XY_STEPS * scale)),
        down_steps=max(30, round(col.DEFAULT_DOWN_STEPS * scale)),
        ...
        settle_steps=(45 if fast else col.DEFAULT_SETTLE_STEPS),
        grasp_steps=(25 if fast else col.DEFAULT_GRASP_STEPS),
        post_success_hold_steps=(5 if fast else col.DEFAULT_POST_SUCCESS_HOLD_STEPS),
        max_action=(0.90 if fast else col.DEFAULT_MAX_ACTION),
...
def run_scripted_grasp_in_wrapped_env(env, object_name, render_callback=None, **_ignored):
    ...
    # PickUpSkill stores the live navigation scene on both wrapper and raw env.
    # Reapply it after the collector-compatible reset so previously transported
    # objects and the current target stay exactly where physics left them.
    scene_snapshot = (
        getattr(env, "_robot_agent_scene_snapshot", None)
        or getattr(raw, "_robot_agent_scene_snapshot", None)
    )
    if scene_snapshot:
        restored = 0
        for joint, qpos in scene_snapshot.items():
            try:
                raw.sim.data.set_joint_qpos(joint, np.asarray(qpos, dtype=float))
                try:
                ...
                logger.warning("scripted grasp: cannot restore scene joint %s", joint)
        raw.sim.forward()
        print(
            f"[SCRIPTED-GRASP] restored {restored} live object pose(s) after reset",
            flush=True,
        )
    robot = raw.robots[0]
    setattr(robot, col.CAMERA_HOLD_TARGET_ATTR, col.capture_camera_hold_targets(robot))
    args = _default_args(col)
    # The official collector records observations internally but exposes no
    # per-step callback. Wrap its single stepping primitive once so trajectory
    # capture sees every simulated control step, rather than only phase ends.
    if not getattr(col, "_robot_agent_step_callback_installed", False):
        original_step_with_record = col.step_with_record

        def _step_with_trajectory_callback(
            env_, base_env_, action_, obs_buffer_, render_, motion_args_
        ):
            result = original_step_with_record(
                env_, base_env_, action_, obs_buffer_, render_, motion_args_
            )
            callback = getattr(motion_args_, "_robot_agent_step_callback", None)
            if callback is not None:
                callback()
            return result

        col.step_with_record = _step_with_trajectory_callback
        col._robot_agent_step_callback_installed = True
    args._robot_agent_step_callback = render_callback
