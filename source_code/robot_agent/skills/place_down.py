...
class PlaceDownSkill(BaseSkill):
    ...
    def run(self, context: ExecutionContext) -> SkillResult:
        ...
        # Fail closed: placement is valid only when the controller physically
        # lowers the held object, opens the grippers, and lets contact settle.
        return SkillResult(
            skill_name=self.name,
            success=False,
            message=f"Physics placement unavailable: {target}",
            payload={
                "action": "place_down",
                "target": target,
    ...
    def _configure_multi_object_drop(self, target: str):
        ...
        if target != "aux_output_1" or self._scene is None:
            return None
        # Keep the three 0.60 m-wide totes physically disjoint while remaining
        # inside the official 0.80 m target radius.
        lateral = (0.0, 0.65, -0.65)[self._drop_index % 3]
        self._drop_index += 1
        if abs(lateral) < 1e-9:
            return None
        try:
