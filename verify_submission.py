...
SUBMISSIONS = ROOT / "submissions"                    # the five official ZIPs
...
# Graded-object overrides. L3 follows the official docx erratum (the blue
# transfer bin is graded, not the task_config baseline object); this mirrors
# JCIIOT/pipeline/finalize_all.ps1 so re-scoring matches the packaged score.json.
OBJ_OVERRIDE = {2: "blue_tote_b01_near_left"}
...
def main() -> int:
    ...
    for lvl, idx in LEVELS:
        ...
        if not zips:
            rows.append((lvl, "<missing zip>", "-", "-", MAX[idx], "FAIL"))
            ok_all = False
            continue
        zpath = zips[-1]
        with zipfile.ZipFile(zpath) as z:
            ...
            try:
                reported = int(json.loads(z.read("score.json")).get("score"))
            except Exception:
                reported = None
        ...
        with tempfile.NamedTemporaryFile("w", suffix=".json", delete=False, encoding="utf-8") as tf:
            ...
            tmp = Path(tf.name)
        try:
            result = st.score(idx, tmp, object_override=OBJ_OVERRIDE.get(idx, ""))
        finally:
            tmp.unlink(missing_ok=True)

        recomputed = int(result.get("total", -1))
        mx = MAX[idx]
        row_ok = (recomputed == mx) and (reported == mx)
        ok_all = ok_all and row_ok
        grand_reported += reported or 0
        grand_recomputed += recomputed
        rows.append((lvl, zpath.name, reported, recomputed, mx, "PASS" if row_ok else "FAIL"))

    print()
    print(f"{'Level':<6}{'Package':<28}{'Reported':>9}{'Recomputed':>12}{'Max':>5}  Result")
    print("-" * 74)
    for lvl, pkg, rep, rec, mx, status in rows:
        print(f"{lvl:<6}{pkg:<28}{str(rep):>9}{str(rec):>12}{mx:>5}  {status}")
    print("-" * 74)
    print(f"{'TOTAL':<34}{grand_reported:>9}{grand_recomputed:>12}{sum(MAX.values()):>5}")
    print()

    if ok_all and grand_recomputed == 100:
